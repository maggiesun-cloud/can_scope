...
class SocketCanInterface(CanInterface):
    """
    Linux SocketCAN driver wrapper using python-can.
    Supports native SocketCAN interfaces (can0, can1, vcan0, etc.)
    and standard/FD frames.
    """
    ...
    def __init__(self):
        ...
        self._running = False
        self._config = CanConfigModel(backend="socketcan", channel="can0")
    ...
    def connect(self, config: CanConfigModel) -> bool:
        if not PYTHON_CAN_AVAILABLE:
            raise RuntimeError("python-can package is not installed.")
        ...
        try:
            ...
            self._connected = False
            raise RuntimeError(f"SocketCAN connection failed on {config.channel}: {str(e)}")
    ...
    def get_status(self) -> BusStatusModel:
        ...
        return BusStatusModel(
            ...
            backend="socketcan",
            interfaceName=f"SocketCAN ({self._config.channel})",
